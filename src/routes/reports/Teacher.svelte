<script>
    ...
    let update=async()=>{
        ...
        let response = await fetch('/edge/distinct', {
                method: 'POST',
                body: JSON.stringify({collection:'reports',match:{coid:status.cycle._id,"author.type":"teacher","author.tid":data.user,type:"A"},aggregate:['fm','g','ss','sc','sl']}),
    ...
    let updateReports=async()=>{
        ...
        if(gp) {
            ...
            console.log(reports.filter((/** @type {{ author: { tid: string; }; g: string; }} */ el)=>el.author.tid===data.user && el.g==='6/C/D'));
            ...
            for(let pupil of gp.pupil) {
                //console.log(data.user,gp.g,pupil.pn,pupil.sn);
                // find the report
                let f=reports.find((/** @type {{ author: { type: string; tid: any; }; pupil: { pid: number; }; g: any; ss: string; sc: string; }} */ el)=>el.author.type==='teacher' && el.author.tid===data.user && el.pupil.pid===pupil.pid && el.g===gp.g);
                console.log(`*${data.user}*`,gp.g,pupil.pn,pupil.sn,`${f?'OK':'NOT FOUND'}`);
        ...
        data.reports=data.reports.sort((/** @type {{ g: string; pupil: { sn: string; pn: string; }; }} */ a,/** @type {{ g: any; pupil: { sn: any; pn: any; }; }} */ b)=>a.g.localeCompare(b.g) || a.pupil.sn.localeCompare(b.pupil.sn) || a.pupil.pn.localeCompare(b.pupil.pn) );
