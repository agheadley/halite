...
let getSubject=(subject)=>{
    ...
    if(cfg.view.chance){    
        txt+=`<div>`;
        txt+=`<div class="report-information-left">
            <div><span class="tag is-small">${subject.chance.A.std}</span></div>
            <div>${getChance(subject.chance.A)}</div>
            <div><span class="tag is-small">${subject.chance.B.std}</span></div>
	...
	for(let item of subject.report) {
		...
		txt+=`<div class="report-information"><div><span>${item.sal}</span></div><div><span class="tag is-small">EFFORT ${item.ec} ${item.ep}</span></div></div>`;
	}
	txt+='</div></div><hr/>';
	txt+=`</section>`;
